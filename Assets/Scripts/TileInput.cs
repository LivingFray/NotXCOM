...
public class TileInput : MonoBehaviour {
            ...
            for (int x = minIntPos.x; x <= maxIntPos.x; x++) {
                for (int y = minIntPos.y; y <= maxIntPos.y; y++) {
                    for (int z = minIntPos.z; z <= maxIntPos.z; z++) {
                        PlaceTile(holder.transform, new Vector3Int(x, y, z));
                    }
                }
            }
    ...
    static void PlaceTile(Transform parent, Vector3Int pos) {
        GameObject tilePrefab = Resources.Load<GameObject>("Tile");
        GameObject inst = Instantiate(tilePrefab, pos, Quaternion.identity, parent);
    }
}
